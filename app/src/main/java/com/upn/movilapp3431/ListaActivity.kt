...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val seriesRef = database.getReference("series") // nodo donde guardarás las series

    val listaSeries = LlenadoDeDatosPruebas().getSeries()

    // Subir cada Serie tal cual, Firebase genera su propio key
    for (serie in listaSeries) {
        seriesRef.push().setValue(serie)
    }
}
